...
struct emac_wiznet_s {
    ...
    bool sock_started;              /*!< SOCK0 was opened by emac_wiznet_start() */
    void *context;                  /*!< Chip-specific context data (size from ops->context_size) */
};
...
static esp_err_t wiznet_drain_rx_buffer(emac_wiznet_t *emac);
...
esp_err_t emac_wiznet_start(esp_eth_mac_t *mac)
{
    ...
    /* open SOCK0 */
    ESP_GOTO_ON_ERROR(wiznet_send_command(emac, ops->cmd_open, WIZNET_SOCK_CMD_GUARD_MS),
                      err, emac->tag, "issue OPEN command failed");
    emac->sock_started = true;
...
esp_err_t emac_wiznet_stop(esp_eth_mac_t *mac)
{
    ...
    if (!emac->sock_started) {
        return ESP_OK;
    }

    /* Let any in-flight TX finish so CLOSE does not race with SEND. */
    for (int i = 0; i < 100; i++) {
        ESP_GOTO_ON_ERROR(wiznet_read(emac, ops->reg_sock_ir, &status, sizeof(status)),
                          err, emac->tag, "read SOCK IR failed");
        if (status & ops->sir_send) {
            uint8_t clr = ops->sir_send;
            ESP_GOTO_ON_ERROR(wiznet_write(emac, ops->reg_sock_irclr, &clr, sizeof(clr)),
                              err, emac->tag, "clear SEND IR failed");
            break;
        }
        vTaskDelay(pdMS_TO_TICKS(10));
    }

    /* Discard pending RX so the next OPEN starts on a clean boundary. */
    if (wiznet_drain_rx_buffer(emac) != ESP_OK) {
        ESP_LOGW(emac->tag, "RX drain failed, continuing with CLOSE");
    }
    ...
    ESP_GOTO_ON_ERROR(wiznet_write(emac, ops->reg_simr, &simr, sizeof(simr)), err, emac->tag, "write SIMR failed");
    /* close SOCK0 */
    ESP_GOTO_ON_ERROR(wiznet_send_command(emac, ops->cmd_close, WIZNET_SOCK_CMD_GUARD_MS),
                      err, emac->tag, "issue CLOSE command failed");
err:
    emac->sock_started = false;
    return ret;
}
...
esp_err_t wiznet_send_command(emac_wiznet_t *emac, uint8_t command, uint32_t timeout_ms)
{
    ...
    ESP_GOTO_ON_ERROR(wiznet_write(emac, ops->reg_sock_cr, &command, sizeof(command)),
                      err, emac->tag, "write SCR failed");

    /* Sn_CR auto-clears when the chip accepts the command (datasheet §4.2.3). */
    do {
        ESP_GOTO_ON_ERROR(wiznet_read(emac, ops->reg_sock_cr, &cr, sizeof(cr)),
                          err, emac->tag, "read SCR failed");
        if (cr == 0) {
            break;
        }
        if (esp_timer_get_time() >= deadline) {
            ESP_GOTO_ON_FALSE(false, ESP_ERR_TIMEOUT, err, emac->tag, "command accept timeout");
        }
        vTaskDelay(pdMS_TO_TICKS(1));
    } while (true);

    /* CLOSE: wait until Sn_SR reports SOCK_CLOSED (fixes W6100 stop under load). */
    if (command == ops->cmd_close) {
        do {
            ESP_GOTO_ON_ERROR(wiznet_read(emac, ops->reg_sock_sr, &sr, sizeof(sr)),
                              err, emac->tag, "read SOCK SR failed");
            if (sr == WIZNET_SOCK_SR_CLOSED) {
                return ESP_OK;
            }
            if (esp_timer_get_time() >= deadline) {
                ESP_GOTO_ON_FALSE(false, ESP_ERR_TIMEOUT, err, emac->tag,
                                  "command complete timeout (Sn_SR=0x%02x, expected 0x%02x)", sr, WIZNET_SOCK_SR_CLOSED);
            }
            vTaskDelay(pdMS_TO_TICKS(1));
        } while (true);
    }

    /* OPEN/RECV/SEND: Sn_CR accept is enough. */
    return ESP_OK;
err:
    return ret;
}
...
/* Reset RX buffer by syncing RX_RD to chip-managed RX_WR. */
static esp_err_t wiznet_drain_rx_buffer(emac_wiznet_t *emac)
{
    esp_err_t ret = ESP_OK;
    const wiznet_chip_ops_t *ops = emac->ops;
    ...
    if (ops->reg_sock_rx_wr == 0) {
        return ESP_OK;
    }
    ESP_GOTO_ON_ERROR(wiznet_read(emac, ops->reg_sock_rx_wr, &rx_wr, sizeof(rx_wr)),
                      err, emac->tag, "read RX WR failed");
    /* RX_RD and RX_WR are big-endian; forward the read value as-is. */
    ESP_GOTO_ON_ERROR(wiznet_write(emac, ops->reg_sock_rx_rd, &rx_wr, sizeof(rx_wr)),
                      err, emac->tag, "write RX RD failed");
    ESP_GOTO_ON_ERROR(wiznet_send_command(emac, ops->cmd_recv, WIZNET_SOCK_CMD_GUARD_MS),
                      err, emac->tag, "issue RECV command failed");
...
static esp_err_t emac_wiznet_alloc_recv_buf(emac_wiznet_t *emac, uint8_t **buf, uint32_t *length)
{
    ...
    *buf = NULL;
    ...
    if (remain_bytes) {
        ...
        /* Reject implausible frame length; triggers buffer drain in RX task. */
        ESP_GOTO_ON_FALSE(rx_len >= ETH_MIN_PACKET_SIZE - ETH_CRC_LEN && rx_len <= ETH_MAX_PACKET_SIZE,
                          ESP_ERR_INVALID_SIZE, err, emac->tag,
                          "implausible frame length %" PRIu16 " from chip header", rx_len);
...
esp_err_t emac_wiznet_receive(esp_eth_mac_t *mac, uint8_t *buf, uint32_t *length)
{
    ...
    if (*length != WIZNET_ETH_MAC_RX_BUF_SIZE_AUTO) {
        ...
        if (remain_bytes) {
            ...
            ESP_GOTO_ON_ERROR(wiznet_read_buffer(emac, &rx_len, sizeof(rx_len), offset), err, emac->tag, "read frame header failed");
            rx_len = __builtin_bswap16(rx_len) - 2; // data size includes 2 bytes of header
            /* Same check as emac_wiznet_alloc_recv_buf. */
            ESP_GOTO_ON_FALSE(rx_len >= ETH_MIN_PACKET_SIZE - ETH_CRC_LEN && rx_len <= ETH_MAX_PACKET_SIZE,
                              ESP_ERR_INVALID_SIZE, err, emac->tag,
                              "implausible frame length %" PRIu16 " from chip header", rx_len);
            copy_len = rx_len > *length ? *length : rx_len;
...
static esp_err_t emac_wiznet_flush_recv_frame(emac_wiznet_t *emac)
{
    ...
    if (remain_bytes) {
        ...
        ESP_GOTO_ON_ERROR(wiznet_write(emac, ops->reg_sock_rx_rd, &offset, sizeof(offset)), err, emac->tag, "write RX RD failed");
        /* issue RECV command */
        ESP_GOTO_ON_ERROR(wiznet_send_command(emac, ops->cmd_recv, WIZNET_SOCK_CMD_GUARD_MS), err, emac->tag, "issue RECV command failed");
        // check if there're more data need to process
        remain_bytes -= rx_len;
        emac->packets_remain = remain_bytes > 0;
    }
        ...
        if (status & ops->sir_recv) {
            ...
            do {
                ...
                if ((ret = emac_wiznet_alloc_recv_buf(emac, &buffer, &frame_len)) == ESP_OK) {
                    ...
                    /* Corrupt header; drain RX buffer to resync. */
                    ESP_LOGE(emac->tag, "RX buffer corruption detected, draining");
                    wiznet_drain_rx_buffer(emac);
                } else {
                               ...
                               const char *task_name)
{
    ...
    /* Allocate chip-specific context data if requested */
    if (ops->context_size > 0) {
        emac->context = calloc(1, ops->context_size);
        if (!emac->context) {
            ESP_LOGE(tag, "no mem for chip-specific context");
            free(emac);
            return NULL;
        }
    }
    ...
    if (ops->add_mac_filter) {
        emac->parent.add_mac_filter = ops->add_mac_filter;
    }
    if (ops->rm_mac_filter) {
        emac->parent.rm_mac_filter = ops->rm_mac_filter;
    }
    if (ops->set_all_multicast) {
        emac->parent.set_all_multicast = ops->set_all_multicast;
    }
#endif
